...
const useMagneticAllowed = (magnetic, disabled) =>
  useMemo(() => {
...
export const Button = ({
  ...
  variant = 'primary',
  className = '',
  magnetic = true, // ✅ NEW (set magnetic={false} for grid buttons)
  disabled = false,
  ...props
}) => {
  const buttonRef = useRef(null);
  ...
  const magneticAllowed = useMagneticAllowed(magnetic, disabled);
  ...
  return (
    <button
      ...
      className={`
        px-6 sm:px-9 py-3 sm:py-3.5 rounded-pill text-sm sm:text-base font-semibold
        inline-flex items-center justify-center gap-3
        transition-all duration-100 ease-out cursor-pointer relative z-10
        ${!disabled ? 'hover-lift' : ''}
        ${disabled ? 'opacity-60 cursor-not-allowed' : ''}
        ${variants[variant]}
        ${magneticAllowed && mousePos.x === 0 ? 'transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]' : ''}
        ${className}
      `}
      {...props}
    >
      <span
        className="pointer-events-none flex w-full items-center justify-center gap-2 transition-transform"
        style={
          magneticAllowed
